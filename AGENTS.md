...
- stores local poster-update completion metadata and movie blacklist under `~/.config/posters/metadata.json`
...
- supports dry-run, force refresh, status view, persistent movie blacklist, cancellation, and explicit Wikipedia fallback mode
...
- keep Go files focused and preferably under 500 lines; split by responsibility when files grow
...
- preserve the modular file layout; avoid rebuilding monolithic `service.go` or `model.go` files
